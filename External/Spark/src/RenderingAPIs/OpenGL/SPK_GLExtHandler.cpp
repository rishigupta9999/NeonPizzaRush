	...
	bool GLExtHandler::loadGLExtPointParameter()
	{
		if (pointParameterGLExt == UNCHECKED)
		{
			...
			if (((int(version[0] - '0') == 1)&&(int(version[2] - '0') >= 4))
				||(int(version[0] - '0') > 1))
			{
				SPK_glPointParameterf  = glPointParameterf;//(SPK_PFNGLPOINTPARAMETERFPROC)glGetProcAddress("glPointParameterf");
				SPK_glPointParameterfv = glPointParameterfv;//(SPK_PFNGLPOINTPARAMETERFVPROC)glGetProcAddress("glPointParameterfv");
			}
