...
namespace GL
{
    unsigned char* ConvertQuadsToTriangles(float* inQuadData, int inAttributeSize, int inStride, unsigned long inNumVertices)
    {
        float* newBuffer = (float*)malloc(sizeof(float) * ((inNumVertices * 3) / 2) * inAttributeSize);
        
        int numQuads = (int)(inNumVertices / 4);
        int readStride = inStride == 0 ? inAttributeSize : inStride;
        
        for (int i = 0; i < numQuads; i++)
        {
            float* readBase = inQuadData + readStride * 4 * i;
            float* writeBase = newBuffer + inAttributeSize * 6 * i;
            
            // Copy first three attributes from the quad normally (index 0, 1, 2)
            memcpy(writeBase, readBase, inAttributeSize * sizeof(float));
            writeBase += inAttributeSize;
            memcpy(writeBase, readBase + readStride, inAttributeSize * sizeof(float));
            writeBase += inAttributeSize;
            memcpy(writeBase, readBase + readStride * 2, inAttributeSize * sizeof(float));
            writeBase += inAttributeSize;
            
            // Then copy second triangle (index 2, 3, 0)
            memcpy(writeBase, readBase + (readStride * 2), inAttributeSize * sizeof(float));
            writeBase += inAttributeSize;
            memcpy(writeBase, readBase + (readStride * 3), inAttributeSize * sizeof(float));
            writeBase += inAttributeSize;
            memcpy(writeBase, readBase, inAttributeSize * sizeof(float));
        }
        
        return (unsigned char*)newBuffer;
    }
	...
	void GLQuadRenderer::render(const Group& group)
	{
		...
		case TEXTURE_2D :
			if (getTexture3DGLExt() == SUPPORTED)
				NeonGLDisable(GL_TEXTURE_3D);
			NeonGLEnable(GL_TEXTURE_2D);
			NeonGLBindTexture(GL_TEXTURE_2D,textureIndex);
        ...
        unsigned long numVertices = group.getNbParticles() << 2;
        
        unsigned char* positionBuffer = ConvertQuadsToTriangles(gpuBuffer, 3, 7, numVertices);
        unsigned char* colorBuffer = ConvertQuadsToTriangles(gpuBuffer + 3, 4, 7, numVertices);
        unsigned char* texcoordBuffer = ConvertQuadsToTriangles(textureBuffer, 2, 0, numVertices);
		...
		glVertexPointer(3,GL_FLOAT,0,positionBuffer);
		glColorPointer(4,GL_FLOAT,0,colorBuffer);

        // TODO rework to use triangles
		glDrawArrays(GL_TRIANGLES,0, (numVertices * 3) / 2);
