...
- (void)viewDidLoad
{
  ...
  self.adView = [[FBAdView alloc] initWithPlacementID:@"YOUR_PLACEMENT_ID"
                                               adSize:isIPAD ? kFBAdSizeHeight90Banner : kFBAdSizeHeight50Banner
...
- (void)viewDidLayoutSubviews
{
  ...
  // then set the banner position and size
  CGSize bannerSize = [self.adView sizeThatFits:self.view.bounds.size];

  CGFloat centeredX = (self.view.bounds.size.width - bannerSize.width) / 2;
  CGFloat bottomAlignedY = self.view.bounds.size.height - bannerSize.height;

  self.adView.frame = CGRectMake(centeredX, bottomAlignedY, bannerSize.width, bannerSize.height);
}
