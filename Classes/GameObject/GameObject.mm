...
-(void)UpdateNumRenders;
-(void)InternalSetPosition:(Vector3*)inVector;
-(void)InternalSetPositionX:(float)inX y:(float)inY z:(float)inZ;

@end

@implementation GameObject

@synthesize SoftBody = mSoftBody;
@synthesize RigidBody = mRigidBody;
@synthesize MaxNumRenders = mMaxNumRenders;
@synthesize NumRenders = mNumRenders;
...
-(void)dealloc
{
    ...
    [[PhysicsManager GetInstance] RemoveSoftBody:mSoftBody];
...
-(void)Update:(CFTimeInterval)inTimeStep
{
            ...
            switch(curNode->mPropertyType)
            {
                case GAMEOBJECT_PROPERTY_POSITION:
                {
                    switch(curNode->mAnimateType)
                    {
                        ...
                        {
                            Vector3 position;
                            [curNode->mData.mPath GetValueVec3:&position];
                            
                            [self InternalSetPosition:&position];
                        ...
                        {
                            [self InternalSetPosition:&curNode->mData.mVector];
                            [curArray removeObject:curNode];
    ...
    if (mRigidBody != NULL)
    {
        if (mRigidBody->getCollisionFlags() & btCollisionObject::CF_KINEMATIC_OBJECT)
        {
            Matrix44 transform;
            [self GetLocalToWorldTransform:&transform];
            
            btMatrix3x3 upperRight;
            upperRight.setFromOpenGLSubMatrix(transform.mMatrix);
            
            btVector3 translation(transform.mMatrix[12], transform.mMatrix[13], transform.mMatrix[14]);
            
            btTransform bulletTransform(upperRight, translation);
            mRigidBody->setWorldTransform(bulletTransform);
        }
    }
}
...
-(void)UpdateNumRenders
{
    if (mNumRenders < mMaxNumRenders)
    {
        mNumRenders++;
    }
    
    if ((mNumRenders == mMaxNumRenders) && (mMaxNumRenders > 0))
    {
        mVisible = FALSE;
    }
}
...
-(void)TransformLocalToWorld:(const Vector3*)inVec result:(Vector3*)outVec
{
    Matrix44 ltwTransform;
...
-(void)InternalSetPosition:(Vector3*)inVector
{
    if (self.SoftBody)
    {
        ...
        Sub3(inVector, &derivedPosition, &requiredPosition);
        
        [[PhysicsManager GetInstance] UpdateSoftBody:self.SoftBody withDelta:&requiredPosition];
    }
    else
    {
...
-(void)GetDerivedPosition:(Vector3*)outPosition
{
    btSoftBody::Node* firstFace = mSoftBody->m_faces.at(0).m_n[0];
    
    Vector3 vertexPosition;
    vertexPosition.mVector[x] = firstFace->m_x.x();
    vertexPosition.mVector[y] = firstFace->m_x.y();
    vertexPosition.mVector[z] = firstFace->m_x.z();

    Sub3(&vertexPosition, &mDerivedPositionDelta, outPosition);
}
...
{
    int numAnimations = (int)[mAnimationList[inProperty] count];
...
-(BOOL)AnyPropertyIsAnimating
{
    ...
    for (int curProperty = 0; curProperty < GAMEOBJECT_PROPERTY_NUM; curProperty++)
    {
        if ([self PropertyIsAnimating:(GameObjectProperty)curProperty])
        {
            retVal = TRUE;
            break;
        }
...
-(void)SetProperty:(GameObjectProperty)inProperty withVector:(Vector3*)inVector
{
    ...
    if ([mAnimationList[inProperty] count] == 0)
    {
        switch(inProperty)
        {
            case GAMEOBJECT_PROPERTY_POSITION:
            {
                [self InternalSetPosition:inVector];
                break;
            }
...
-(void)SetUserData:(u64)inUserData
{
...
-(void)setSoftBody:(btSoftBody*)inSoftBody
{
    NSAssert(mRigidBody == NULL, @"Can't set a soft body when a rigid body is already specified");
    ...
    {
        btSoftBody::Node* firstFace = mSoftBody->m_faces.at(0).m_n[0];
        mDerivedPositionDelta.mVector[x] = firstFace->m_x.x();
        mDerivedPositionDelta.mVector[y] = firstFace->m_x.y();
        mDerivedPositionDelta.mVector[z] = firstFace->m_x.z();
        
        [[PhysicsManager GetInstance] UpdateSoftBodyTransform:mSoftBody fromObject:self];

        // We zero out the transforms since that's inherited by the soft body mesh
        Set(&mPosition, 0, 0, 0);
        Set(&mOrientation, 0, 0, 0);
        Set(&mScale, 1, 1, 1);
        SetIdentity(&mLTWTransform);
    }
}
...
-(void)setRigidBody:(btRigidBody*)inRigidBody
{
    NSAssert(mSoftBody == NULL, @"Can't set a rigid body when a soft body is already specified");
    
    mRigidBody = inRigidBody;
}
