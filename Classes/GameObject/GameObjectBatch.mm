...
{
    int numObjects = (int)[mGameObjects count];
...
-(void)moveObjectToFront:(GameObject*)inObject
{
    [inObject retain];
    
    [self removeObject:inObject];
    [mGameObjects insertObject:inObject atIndex:0];
    
    [inObject release];
}

-(void)moveObjectToBack:(GameObject*)inObject
{
    [inObject retain];
