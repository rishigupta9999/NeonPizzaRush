...
-(void)CreateAtlas
{
    ...
    // PVRTC hack.  We can have a PVRTC texture atlas with just one texture entry
    BOOL usePVRTCHack = FALSE;
    
    for (TextureAtlasEntry* entry in mTextureList)
    {
        if ([entry->mTexture class] == [PVRTCTexture class])
        {
            NSAssert([mTextureList count] == 1, @"PVRTC textures can only be used in a 1 texture atlas");
            usePVRTCHack = TRUE;
            
            entry->mTexture->mTextureAtlasInfo.mPlaced = TRUE;
        
            entry->mTexture->mTextureAtlasInfo.mX = 0;
            entry->mTexture->mTextureAtlasInfo.mY = 0;
            
            entry->mTexture->mTextureAtlasInfo.mSMin = 0;
            entry->mTexture->mTextureAtlasInfo.mTMin = 0;
            
            entry->mTexture->mTextureAtlasInfo.mSMax = 1;
            entry->mTexture->mTextureAtlasInfo.mTMax = 1;
            
            mTextureObject = entry->mTexture->mTexName;
        }
    }
    ...
    TextureFitterContext* context = (TextureFitterContext*)malloc(sizeof(TextureFitterContext));
...
{
    unsigned char* buffer = (unsigned char*)malloc(mAtlasWidth * mAtlasHeight * 4);
    ...
    for (Texture* curTexture in inContext->mPlacedTextures)
    {
        ...
        int maxX = NeonMin([curTexture GetMaxWidth], [textureNameTexture GetRealWidth]);
        int maxY = NeonMin([curTexture GetMaxHeight], [textureNameTexture GetRealHeight]);
    ...
    WritePNG(buffer, [NSString stringWithFormat:@"%d.png", (int)[inContext->mPlacedTextures count]], mAtlasWidth, mAtlasHeight);
