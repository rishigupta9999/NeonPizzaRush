...
+(void)InitDefaultParams:(TextTextureParams*)outParams
{
    ...
    SetColor(&outParams->mBackgroundColor, 0, 0, 0, 0);
...
-(BOOL)RenderWithParams_CoreGraphics:(TextTextureParams*)inParams texture:(Texture*)inTexture
{
    ...
    u32 clearColor = CFSwapInt32(GetRGBAU32(&inParams->mBackgroundColor));

#if DEBUG_TEXT_GENERATION
    clearColor = 0x80808080;
#endif

    for (int i = 0; i < (paddedWidth * paddedHeight); i++)
    {
        ((u32*)inTexture->mTexBytes)[i] = clearColor;
    }
    ...
    CGContextRef cgContextRef = CGBitmapContextCreate(inTexture->mTexBytes, paddedWidth, paddedHeight, 8, paddedWidth * 4, mColorSpaceRef, (CGBitmapInfo)kCGImageAlphaPremultipliedLast);
...
{
    int numSpans = (int)[inSpans count];
    ...
    for (int curSpanIndex = 0; curSpanIndex < numSpans; curSpanIndex++)
    {
        ...
        if (nextSpan == NULL)
        {
            spanSize = (int)[inStrippedString length] - curSpan->mPosition;
        }
        else
        {
...
-(NSMutableArray*)GenerateTextSpans:(TextTextureParams*)inParams strippedString:(NSString**)outStrippedString
{
    ...
    int length = (int)[inParams->mString length];
...
-(CGSize)MeasureFrame:(CTFrameRef)frame startX:(u32*)outStartX startY:(u32*)outStartY endX:(u32*)outEndX endY:(u32*)outEndY
{
    ...
    *outStartY = NeonMax((int)(textHeight - points[0].y - firstLineHeight), 0);
