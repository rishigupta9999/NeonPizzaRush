...
{
    u8* newTex = (u8*)malloc(inDestHeight * inDestWidth * 4);
...
{
    int numLevels = NeonMax(log(mWidth) / log(2.0), log(mHeight) / log(2.0)) + 1;
    ...
    if (inLevel == 0)
    {
        ...
        mSrcMipMapTexBytes = (u8**)malloc(sizeof(u32*) * numLevels);
        memset(mSrcMipMapTexBytes, 0, sizeof(u32*) * numLevels);
        
        double heightPow = log2(mHeight);
        double widthPow = log2(mWidth);
        
        numLevels = NeonMax(ceil(widthPow), ceil(heightPow)) + 1;

        mMipMapTexBytes = (u8**)malloc(sizeof(u32*) * numLevels);
...
{
    return (NeonMax(log(mGLWidth) / log(2.0), log(mGLHeight) / log(2.0)) + 1);
}

-(u32)GetNumSrcMipMapLevels
{
    return (NeonMax(log(mWidth) / log(2.0), log(mHeight) / log(2.0)) + 1);
}
