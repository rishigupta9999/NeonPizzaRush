...
-(TextureButton*)InitWithParams:(TextureButtonParams*)inParams
{
    ...
    if (inParams->mButtonText != NULL)
    {
        ...
        [mTextTexture SetScaleFactor:scaleFactor];
        
        mTextStartX = params.mStartX / GetTextScaleFactor();
        mTextStartY = params.mStartY / GetTextScaleFactor();
        mTextEndX = params.mEndX / GetTextScaleFactor();
        mTextEndY = params.mEndY / GetTextScaleFactor();
...
-(BOOL)HitTestWithPoint:(CGPoint*)inPoint
{
    ...
    {
        if  (   (inPoint->x >= -mParams.mBoundingBoxBorderSize.mVector[x]) &&
                (inPoint->y >= -mParams.mBoundingBoxBorderSize.mVector[y]) &&
                (inPoint->x < ([useTexture GetEffectiveWidth] + mParams.mBoundingBoxBorderSize.mVector[x])) &&
                (inPoint->y < ([useTexture GetEffectiveHeight] + mParams.mBoundingBoxBorderSize.mVector[y]))    )
        {
            buttonTouched = TRUE;
        }
    }
...
-(u32)GetWidth
{
    ...
    if (mBaseTexture != NULL)
    {
        retWidth = [mBaseTexture GetEffectiveWidth];
    }
    else
    {
        retWidth = mScale.mVector[x];
    }
...
-(u32)GetHeight
{
    ...
    if (mBaseTexture != NULL)
    {
        retHeight = [mBaseTexture GetEffectiveHeight];
    }
    else
    {
        retHeight = mScale.mVector[y];
    }
