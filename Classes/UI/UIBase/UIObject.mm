...
@interface UIObject(Private)

-(void)AddChild:(UIObject*)inChild;
...
@synthesize ColorMultiplyEnabled = mColorMultiplyEnabled;
...
{    
    ...
    SetColorFloat(&mColorMultiply, 1.0, 1.0, 1.0, 1.0);
...
-(void)GetLocalToWorldTransform:(Matrix44*)outTransform
{
    ...
    Vector3 curScale;
    [self GetPropertyCurrent:GAMEOBJECT_PROPERTY_SCALE withVector:&curScale];
    
    float xScale = curScale.mVector[x];
    float yScale = curScale.mVector[y];

    mPosition.mVector[x] -= hAlign * xScale;
    mPosition.mVector[y] -= vAlign * yScale;
    ...
    UIObject* curParent = self.Parent;

    if (curParent != NULL)
    {
        Matrix44 parentTransform;
        [curParent GetLocalToWorldTransform:&parentTransform];
        
        MatrixMultiply(&parentTransform, outTransform, outTransform);
    }
}
...
-(void)GetWorldPosition:(Vector3*)outPosition
{
    [self GetPosition:outPosition];
    
    if (self.Parent)
    {
        Matrix44 parentTransform;
        [self.Parent GetLocalToWorldTransform:&parentTransform];
        
        Vector4 outVec;
        TransformVector4x3(&parentTransform, outPosition, &outVec);
        SetVec3From4(outPosition, &outVec);
    }
}
...
-(float)GetCombinedAlpha
{
    ...
    return [mParent GetCombinedAlpha] * mAlpha;
}
...
-(void)SetVisible:(BOOL)inVisible
{
    ...
    for (UIObject* curChild in mChildren)
    {
        [curChild SetVisible:inVisible];
    }
}
...
-(Texture*)LoadTextureWithParams:(UIObjectTextureLoadParams*)inParams
{
    ...
    GLenum filter = [UIObject GetOrthoTexFilter];
    
    TextureParams textureParams;
    [Texture InitDefaultParams:&textureParams];
    
    textureParams.mTextureAtlas = atlas;
    textureParams.mMinFilter = filter;
    textureParams.mMagFilter = filter;
...
-(void)RegisterTexture:(Texture*)inTexture
{
    ...
    inTexture->mParams.mMinFilter = filter;
    inTexture->mParams.mMagFilter = filter;
...
+(void)InitQuadParams:(QuadParams*)outQuadParams
{
    ...
    outQuadParams->mRotation = 0;
    outQuadParams->mPositionCoordsEnabled = FALSE;
...
-(void)EvaluateDirtyBits:(QuadParams*)inQuadParams
{
...
-(void)DrawQuad:(QuadParams*)inQuadParams
{
    ...
    [self EvaluateDirtyBits:inQuadParams];
    
    if (inQuadParams->mPositionCoordsEnabled)
    {
        memcpy(vertex, inQuadParams->mPositionCoords, sizeof(float) * 12);
    }
    ...
    Matrix44 rotation;
    GenerateRotationMatrix(inQuadParams->mRotation, 0, 0, 1, &rotation);
    
    MatrixMultiply(&translate, &rotation, &transform);
    MatrixMultiply(&transform, &scale, &transform);
    ...
    if ((inQuadParams->mTexture != NULL) && (inQuadParams->mTexture->mParams.mTextureAtlas != NULL))
    {
        ...
        if (meshBuilder != NULL)
        {
            [meshBuilder StartMeshWithOwner:self];
...
-(void)PerformWhenLoaded:(dispatch_queue_t)inQueue block:(dispatch_block_t)inBlock
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^
    {
        while(true)
        {
            ...
            if ([[self GetUseTexture] GetStatus] != TEXTURE_STATUS_DECODING_COMPLETE)
            {
                complete = FALSE;
                break;
            }
...
+(void)PerformWhenAllLoaded:(NSMutableArray*)inObjects queue:(dispatch_queue_t)inQueue block:(dispatch_block_t)inBlock
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^
    {
        while(true)
        {
            ...
            for (UIObject* curObject in inObjects)
            {
                NSAssert([curObject GetUseTexture] != NULL, @"Can't have a NULL texture here");
                
                if ([[curObject GetUseTexture] GetStatus] != TEXTURE_STATUS_DECODING_COMPLETE)
                {
                    complete = FALSE;
                    break;
                }
...
-(void)setParent:(UIObject*)inParent
{
