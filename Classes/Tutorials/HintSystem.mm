...
HintInfo    sHintInfo[HINT_ID_NUM]  = { { @"HINT_ID_PERFORM_SPIN",   EVENT_INCREMENTAL_GAME_START, GAMEMODE_TYPE_INVALID,   0,  EVENT_EMPTY,  0, HintCondition_Equals, HINTTYPE_ONCE_SESSION,  NULL, 0,   0, NULL  }
                                        };
    ...
    switch(mState)
    {
        case HINTSYSTEM_STATE_IDLE:
        {
            for (int i = 0; i < HINT_ID_NUM; i++)
            {
                ...
                if ((sHintInfo[i].mTriggerEvent == inMsg->mId) && (triggerGameMode) && (![self GetHintDisplayed:(HintId)i]))
                {
                    ...
                    if (triggerEvaluator)
                    {
                        ...
                        {
                            [self DisplayHint:(HintId)i];
                            mActiveHint = (HintId)i;
                        }
                        ...
                        [self SetHintDisplayed:(HintId)i];
