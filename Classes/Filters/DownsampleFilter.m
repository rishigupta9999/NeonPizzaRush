...
-(Filter*)InitWithTexture:(Texture*)inTexture numLevels:(int)inNumLevels
{
    ...
    int numLevels = log(NeonMax([inTexture GetGLHeight], [inTexture GetGLWidth])) / log(2.0);
    mNumLevels = NeonMin(numLevels, inNumLevels);
    ...
    for (int curDownsample = 0; curDownsample < mNumLevels; curDownsample++)
    {
        ...
        createParams.mWidth = NeonMax(MIN_FRAMEBUFFER_DIMENSION, width);
        createParams.mHeight = NeonMax(MIN_FRAMEBUFFER_DIMENSION, height);
...
-(void)Update:(CFTimeInterval)inTimeStep
{
    ...
    glPushMatrix();
    {
        ...
        glPushMatrix();
        {
            ...
            int width = NeonMax([mSourceTexture GetGLWidth] / 2, 1);
            int height = NeonMax([mSourceTexture GetGLHeight] / 2, 1);
            ...
            for (int curLevel = 0; curLevel < mNumLevels; curLevel++)
            {
                ...
                width = NeonMax(width / 2, 1);
                height = NeonMax(height / 2, 1);
