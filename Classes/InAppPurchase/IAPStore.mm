...
-(IAPStore*)Init
{
    ...
    // Unlock next room
    mIAPRowDefinitions[2] = [[PowerupInfo alloc] init];
    
    mIAPRowDefinitions[2].OverlayFilename = @"run21_unlockroom_overlay.papng";
    mIAPRowDefinitions[2].DescriptionString = @"LS_Unlock_Room_Description";
    mIAPRowDefinitions[2].DescriptionStringTimed = @"LS_Unlock_Room_Description";
    
    mIAPRowDefinitions[2].QuantityRequiresUpdate = TRUE;
    mIAPRowDefinitions[2].QuantityStringFontSize = 10;
    mIAPRowDefinitions[2].QuantityString = @"GetRoomUnlockDescription:";
    mIAPRowDefinitions[2].QuantityAccessor = @"GetMaxRoomUnlocked";
    mIAPRowDefinitions[2].Unlocked = @"GetRoomsUnlocked";
    mIAPRowDefinitions[2].UnlockLevel = @"GetRoomsUnlockLevel";
    mIAPRowDefinitions[2].PowerupName = @"LS_Unlock_Room";
    
    SetVec2(&mIAPRowDefinitions[2]->mQuantityStringPosition, 100, 20);
    
    mIAPRowDefinitions[2].PowerupPurchaseSelector = @"GetRoomUnlockString:";
    mIAPRowDefinitions[2].ActiveSelector = @"GetRoomUnlockButtonActive:";
    mIAPRowDefinitions[2].HasRestoreButton = TRUE;
...
-(void)ProcessEvent:(EventId)inEventId withData:(void*)inData
{
    ...
    switch(inEventId)
    {
        case EVENT_IAP_DELIVER_CONTENT:
        {
            ...
            IapProduct product                          = [inAppPurchaseManager GetIAPWithIdentifier:(NSString*)inData];
...
-(void)LeaveMenu
{
    int numObjects = (int)[mUIObjects count];
...
{
    LevelDefinitions* levelDefinitions = [Flow GetInstance].LevelDefinitions;
    ...
    for (int curProduct = 0; curProduct < IAPSTORE_PRODUCT_NUM; curProduct++)
    {
        ...
        BOOL productLocked = ([levelDefinitions performSelector:productUnlockedSelector] == NULL);
        ...
        tbParams.mAlignment     = kCTTextAlignmentLeft;
...
-(void)ActivateRowBackground:(int)inRowIndex
{
    ...
    int unlockLevel = (int)((u64)[levelDefinitions performSelector:productUnlockLevel] + 1);
...
-(NSString*)GetPowerupQuantityDescription:(NSNumber*)inRow
{
    ...
    u64 quantity = (u64)[[SaveSystem GetInstance] performSelector:getter];
...
-(BOOL)EvaluateButtonActiveForProduct:(int)inProduct index:(int)inIndex
{
    ...
    if (mIAPRowDefinitions[inProduct].ActiveSelector != NULL)
    {
        ...
        buttonActive = ([self performSelector:NSSelectorFromString(mIAPRowDefinitions[inProduct].ActiveSelector) withObject:productQuantityPair] != NULL);
    }
