...
static IAPInfo sIAPInfo[IAP_PRODUCT_NUM] = { { @"Booster", @"GetBooster", @"SetBooster:", IAP_OPERATOR_SET, 1 } };
        ...
        switch (transaction.transactionState)
        {
            ...
            case SKPaymentTransactionStateFailed:               //The transaction failed. Check the error property to determine what happened.
            {
                ...
                if ( transaction.error.code == SKErrorPaymentCancelled )
                {
                    ...
                    [AdvertisingManager GetInstance].AdsEnabled = TRUE;
                }
                else
                {
                ...
                [[NeonMetrics GetInstance] logEvent:flurryLabel withParameters:[NSDictionary dictionaryWithObjectsAndKeys:
                                             productId, @"IAP Product ID"   ,
                                             [NSString stringWithFormat:@"%ld", (long)transaction.error.code], @"IAP Error",
                                             NULL]];
...
-(void)paymentQueue:(SKPaymentQueue*)queue restoreCompletedTransactionsFailedWithError:(NSArray*)transactions
{
    [InAppPurchaseManager GetInstance]->mIapState = IAP_STATE_IDLE;
}
...
-(InAppPurchaseManager*)Init
{
    ...
    for (int i = 0; i < IAP_PRODUCT_NON_CONSUMABLE_NUM; i++)
    {
        ...
        id retVal = [[SaveSystem GetInstance] performSelector:getter];
        mNonConsumablePurchases[i] = (retVal != NULL);
    }
...
-(BOOL)RequestProduct:(IapProduct)inProduct
{
    ...
    // Disable ads while we could possibly be displaying IAP alerts, otherwise Flurry crashes when dismissing an ad
    [AdvertisingManager GetInstance].AdsEnabled = FALSE;
    ...
    {
        NSLog(@"Invalid product with ID: %d, Products Available: %d :: Repopulating products dictionary on user tap.", inProduct, (int)[mProducts count]);
...
-(void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)inResponse
{
    ...
    [GetGlobalMessageChannel() SendEvent:EVENT_IAP_PRICES_RECEIVED withData:NULL];
...
-(void)DeliverContent:(NSString*)inProductIdentifier
{
    ...
    for (int i = 0; i < IAP_PRODUCT_NUM; i++)
    {
        ...
        if ([sIAPInfo[i].mProductId isEqualToString:inProductIdentifier])
        {
            ...
            u64 numItems = (u64)[[SaveSystem GetInstance] performSelector:getter];
            ...
            [[SaveSystem GetInstance] performSelector:setter withObject:[NSNumber numberWithLong:(long)numItems]];
    ...
    [UIApplication sharedApplication].statusBarOrientation = GAME_ORIENTATION;
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:resultString message:NULL delegate:self cancelButtonTitle:NSLocalizedString(@"LS_OK",NULL) otherButtonTitles:nil];
    [alert show];
    
    [GetGlobalMessageChannel() SendEvent:EVENT_IAP_DELIVER_CONTENT withData:inProductIdentifier];
}
...
-(void)NotifyUserOfError:(NSError*)inError
{
    ...
    [UIApplication sharedApplication].statusBarOrientation = GAME_ORIENTATION;
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"LS_IAP_ERROR_TITLE",NULL) message:errorString delegate:self cancelButtonTitle:NSLocalizedString(@"LS_OK",NULL) otherButtonTitles:nil];
    [alert show];
}
