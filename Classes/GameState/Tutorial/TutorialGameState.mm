...
-(void)Startup
{
    ...
    TutorialScript* newScript = [[Flow GetInstance].LevelDefinitions GetTutorialScript];
...
-(void)Update:(CFTimeInterval)inTimeStep
{
    ...
    switch(mTutorialGameStatePulse)
    {
        ...
        {
            NSAssert(FALSE, @"Unsupported tutorial game state");
            break;
        }
    }
...
-(void)SkipTutorial
{
    mCurrentPhase = (s32)[mTutorialScript->mPhaseInfo count];
    mTutorialGameStateStatus = TUTORIAL_GAME_STATE_NORMAL;
...
-(void)EvaluatePhase
{
    ...
    mTerminateEvent = phaseInfo->mTerminateEvent;
...
-(void)EvaluateCameraRestore:(BOOL)inForce
{
    ...
    if (((phaseInfo != NULL) && (phaseInfo->mRestoreCamera)) || (inForce))
    {
        ...
        NSAssert(FALSE, @"Need to handle this case for non Solitaire 21 games");
...
{
    if ((inEventId == EVENT_ANY_BUTTON_DOWN) || (inEventId == mTerminateEvent))
    {
        [self TerminatePhase];
    }
