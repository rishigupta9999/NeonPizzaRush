...
typedef struct
{
    ...
    u64             mStateActionData;
    u64             mStateActionData2;
} StateAction;
...
static const u32    sButtonFontColors[CARDSUIT_NumSuits]		= { 0x158dfaFF,
																	0xee2929FF,
																	0xe1df24FF,
																	0x33f08fFF};
                                                                            
static const u32    sButtonFontBorderColors[CARDSUIT_NumSuits]  = { 0x0d52dfFF,
                                                                    0x7f1010FF,
                                                                    0xa9a616FF,
                                                                    0x20c55aFF};
...
-(void)LeaveMenu
{
    int  numObjects = (int)[mUIGroup count];
...
-(void)InitMenuToggle:(PauseSubMenuAction)linkMenuID withSuit:(int)suitID withOn:(BOOL)bToggledOn withEnabled:(BOOL)bEnabled
{
    ...
    if (inEvent == BUTTON_EVENT_UP)
    {
        switch (inButton->mIdentifier)
        {
            ...
            case PAUSE_SUB_MENU_RESUME_GAME:
            {
                ...
                StateAction* actions = (StateAction*)malloc(sizeof(StateAction) * 1);
                
                actions[0].mStateActionType = STATE_ACTION_POP;
                actions[0].mStateActionData = 0;
                
                sUserData.mData[0] = (u64)actions;
                sUserData.mData[1] = 1;
...
-(void)Update:(CFTimeInterval)inTimeStep
{
    if ([mUIGroup GroupCompleted] && (sUserData.mType != USERDATA_INVALID))
    {
        ...
        switch(userDataType)
        {
            ...
            {
                int numActions = (int)sUserData.mData[1];
