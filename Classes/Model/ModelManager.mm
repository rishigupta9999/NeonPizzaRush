...
+(void)InitDefaultDrawParams:(ModelManagerDrawParams*)outParams
{
    ...
    outParams->mCondition = ModelManagerCondition_GreaterThan;
    outParams->mPriorityTwo = -2;
    outParams->mConditionTwo = ModelManagerCondition_GreaterThanEquals;
...
{    
    Matrix44 projMatrix, postProjMatrix;
    
    NeonGLMatrixMode(GL_PROJECTION);
    glPushMatrix();
    
    [[CameraStateMgr GetInstance] GetProjectionMatrix:&projMatrix];
    [[CameraStateMgr GetInstance] GetPostProjectionMatrix:&postProjMatrix];
    
    MatrixMultiply(&postProjMatrix, &projMatrix, &projMatrix);
...
-(void)SetupUICamera
{
    ...
    {
        xOffset = ((float)aspect * (float)ADVERTISING_MANAGER_TOP_BANNER_OFFSET);
        yOffset = ADVERTISING_MANAGER_TOP_BANNER_OFFSET;
    }
    
	float halfBaseWidth = (GetScreenAbsoluteWidth() + xOffset) / 2.0f;
	float halfBaseHeight = (GetScreenAbsoluteHeight() + yOffset) / 2.0f;
	...
	glOrthof(   -halfBaseWidth, halfBaseWidth,
                -halfBaseHeight, halfBaseHeight,
                -1.0f, 1.0f );
    ...
    if ([[AdvertisingManager GetInstance] ShouldShowBannerAds])
    {
        offsetY = -ADVERTISING_MANAGER_TOP_BANNER_OFFSET;
        
        glEnable(GL_SCISSOR_TEST);
        glScissor(0, 0, GetScreenAbsoluteWidth() * GetScreenScaleFactor(), (GetScreenAbsoluteHeight() - ADVERTISING_MANAGER_TOP_BANNER_OFFSET) * GetScreenScaleFactor());
    }
    
    glTranslatef(-halfBaseWidth, halfBaseHeight + offsetY, 0.0f);
    ...
    switch(inViewportType)
    {
        case MODELMANAGER_VIEWPORT_ORTHO:
        {
            if (GetDevicePad() || GetDeviceiPhoneTall())
            {
                ...
                NeonGLViewport(0, 0, screenWidth, screenHeight);
            }
        ...
        case MODELMANAGER_VIEWPORT_UI:
        {
            if (GetDevicePad() || GetDeviceiPhoneTall())
            {
                ...
                NeonGLViewport( ((screenWidth - renderWidth) / 2),
                                ((screenHeight - renderHeight) / 2),
...
-(BOOL)ShouldDrawObject:(GameObject*)inObject withParams:(ModelManagerDrawParams*)inParams
{
    ...
    ModelManagerCondition conditions[2] = { inParams->mCondition, inParams->mConditionTwo };
    int priority[2] = { inParams->mPriority, inParams->mPriorityTwo };
    ...
    for (int i = 0; i < 2; i++)
    {
        switch(conditions[i])
        {
            ...
            {
                if (sRenderBins[inObject->mRenderBinId].mPriority <= priority[i])
                {
                    return FALSE;
                }
            ...
            case ModelManagerCondition_GreaterThanEquals:
            {
                if (sRenderBins[inObject->mRenderBinId].mPriority < priority[i])
                {
                    return FALSE;
