...
-(instancetype)InitWithData:(NSData*)inData
{
    ...
    mStream = (unsigned char*)malloc(mStride * mNumVertices);
    ...
    {
        mSkinnedPositions = (float*)malloc(sizeof(float) * 3 * mNumVertices);
        mSkinnedNormals = (float*)malloc(sizeof(float) * 3 * mNumVertices);
    }
    else
    {
...
-(instancetype)Init
{
    ...
    mNumMatricesPerVertex = 0;
    
    mStride = 0;
    
    mJointMatrices = 0;
    mJointMatricesInverseTranspose = 0;
...
{
    NSAssert(mNumMatricesPerVertex == 0, @"Unsupported.  This function requires VBOs to be used, although we could extend it");
    
    Matrix44 inverseTranspose;
    InverseTranspose(inTransform, &inverseTranspose);
    ...
    for (int curVertex = 0; curVertex < mNumVertices; curVertex++)
    {
        ...
        float* writeStream = vertexBase;
        float* normalStream = normalBase;
        ...
        TransformVector4x3(inTransform, &sourceVertex, &transformedVertex);
        TransformVector4x3(&inverseTranspose, &sourceNormal, &transformedNormal);
    ...
    if (mNumMatricesPerVertex != 0)
    {
        ...
        for (int curVertexIndex = 0; curVertexIndex < mNumVertices; curVertexIndex++)
        {
            for (int curMatrix = 0; curMatrix < mNumMatricesPerVertex; curMatrix++)
            {
                ...
                retVal = NeonMax(retVal, testMatrix);
            }
        }
    }
...
{
    NSAssert((mNumVertices % 3) == 0, @"There must be an integer number of triangles (num vertices is a multiple of 3).  The mesh must be laid out as GL_TRIANGLES");
    ...
    {
        int startVertex = curTriangle * 3;
        
        float* position0 = (float*)(mStream + (mStride * startVertex));
        float* position1 = (float*)(mStream + (mStride * (startVertex + 1)));
        float* position2 = (float*)(mStream + (mStride * (startVertex + 2)));
        
        Vector3 positionVector0, positionVector1, positionVector2;
        Set(&positionVector0, position0[0], position0[1], position0[2]);
        Set(&positionVector1, position1[0], position1[1], position1[2]);
        Set(&positionVector2, position2[0], position2[1], position2[2]);
        
        Vector3 intersectionPosition;
        float t;
        BOOL intersect = RayIntersectionWithTriangle(inRayStart, inDirection, &positionVector0, &positionVector1, &positionVector2, &t, &intersectionPosition);
        
        if (intersect)
        {
            Vector3 barycentricCoords;
            BarycentricCoordsInTriangle(&intersectionPosition, &positionVector0, &positionVector1, &positionVector2, &barycentricCoords);
            
            outIntersectionInfo->mIntersectionVertexIndex[intersection] = startVertex;
            outIntersectionInfo->mT[intersection] = t;
            CloneVec3(&intersectionPosition, &outIntersectionInfo->mIntersectionPosition[intersection]);
            
            float* texcoord0 = (float*)(mStream + (mStride * startVertex) + (mPositionStride * sizeof(float)) + (mNormalStride * sizeof(float)));
            float* texcoord1 = (float*)(mStream + (mStride * (startVertex + 1)) + (mPositionStride * sizeof(float)) + (mNormalStride * sizeof(float)));
            float* texcoord2 = (float*)(mStream + (mStride * (startVertex + 2)) + (mPositionStride * sizeof(float)) + (mNormalStride * sizeof(float)));
            
            Vector2 texcoordVector0, texcoordVector1, texcoordVector2;
            SetVec2(&texcoordVector0, texcoord0[0], texcoord0[1]);
            SetVec2(&texcoordVector1, texcoord1[0], texcoord1[1]);
            SetVec2(&texcoordVector2, texcoord2[0], texcoord2[1]);
            
            Scale2(&texcoordVector0, barycentricCoords.mVector[x]);
            Scale2(&texcoordVector1, barycentricCoords.mVector[y]);
            Scale2(&texcoordVector2, barycentricCoords.mVector[z]);
            
            Add2(&texcoordVector0, &texcoordVector1, &outIntersectionInfo->mIntersectionTexcoord[intersection]);
            Add2(&outIntersectionInfo->mIntersectionTexcoord[intersection], &texcoordVector2, &outIntersectionInfo->mIntersectionTexcoord[intersection]);
            
            intersection++;
        }
    }
    
    //NSAssert(((intersection == 0) || (intersection == 2)), @"We don't handle the 1 intersection case at the moment");
