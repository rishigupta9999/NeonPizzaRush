...
-(CameraStateMachine*)Init
{
    ...
    SetIdentity(&mPostProjectionMatrix);
    ...
    SetIdentity(&mInversePostProjectionMatrix);
...
-(void)CacheCameraParameters
{
    ...
    [(CameraState*)mActiveState GetPostProjectionMatrix:&mPostProjectionMatrix];
    
    Inverse(&mViewMatrix, &mInverseViewMatrix);
    InverseProjection(&mProjectionMatrix, &mInverseProjectionMatrix);
    Inverse(&mPostProjectionMatrix, &mInversePostProjectionMatrix);
...
{
    CloneMatrix44(&mPostProjectionMatrix, outPostProjectionMatrix);
}
...
{
    CloneMatrix44(&mInversePostProjectionMatrix, outInversePostProjectionMatrix);
}
...
+(float)GetRequiredHFovForRect:(Rect3D*)inRect position:(Vector3*)inPosition lookAt:(Vector3*)inLookAt
{
    ...
    float width = 2.0 * NeonMax(fabsf(maxX), fabsf(minX));
    float height = 2.0 * NeonMax(fabsf(maxY), fabsf(minY));
