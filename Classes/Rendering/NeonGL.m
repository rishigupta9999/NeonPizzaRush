...
-(void)BindFramebuffer:(GLenum)inTarget identifier:(int)inIdentifier
{
    ...
    glBindFramebufferOES(inTarget, inIdentifier);
}

-(void)DeleteFramebufferWithIdentifier:(GLuint)inIdentifier
{
    if (mFramebuffer == inIdentifier)
    {
        mFramebuffer = TEXTURE_INVALID;
    ...
    glDeleteFramebuffersOES(1, &inIdentifier);
}
...
{
    NSCAssert(inNumFramebuffers == 1, @"This call is only supported with one framebuffer at a time");
    
    [[NeonGL GetInstance] DeleteFramebufferWithIdentifier:inIdentifiers[0]];
}