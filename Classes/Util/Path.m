...
-(instancetype)InitWithPath:(Path*)inPath
{
...
{    
    ...
    else
    {
        ...
        int index = (int)[mNodes indexOfObject:lower];
...
-(void)AddNodeX:(float)inX y:(float)inY atTime:(CFTimeInterval)inTime
{
    [self AddNodeX:inX y:inY z:0 atTime:inTime];
}
...
-(void)EvaluatePath
{
    if (mType == PATH_TYPE_TIME)
    {
        int actionCount = (int)[mActions count];
