...
{
    float a = NeonMin(inX, inY);
    return NeonMin(a, inZ);
}
...
void Bresenham(int x1, int y1, int x2, int y2, BresenhamCallback inCallback, void* inBresenhamUserData)
{
    int slope;
    ...
    if (x1 > x2)
    {
        Bresenham(x2, y2, x1, y1, inCallback, inBresenhamUserData);
        return;
    }
    ...
    for (x = x1; x <= x2; x++)
    {
        inCallback(x, y, inBresenhamUserData);
...
void BarycentricCoordsInTriangle(Vector3* inPoint, Vector3* inTri0, Vector3* inTri1, Vector3* inTri2, Vector3* outPoint)
{
    ...
    Sub3(inTri1, inTri0, &v0);
    Sub3(inTri2, inTri0, &v1);
    Sub3(inPoint, inTri0, &v2);
