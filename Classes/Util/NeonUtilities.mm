...
void SaveScreenRect(NSString* inFilename, int inWidth, int inHeight)
{
    ...
    u8* buffer = (u8*)malloc(inWidth * inHeight * 4);
...
void SaveGLState(GLState* inState)
{
    ...
    glGetIntegerv(GL_DEPTH_WRITEMASK, &inState->mDepthMaskEnabled);
...
void RestoreGLState(GLState* inState)
{
    ...
    glDepthMask(inState->mDepthMaskEnabled);
...
u32 GetScreenAbsoluteWidth()
{
    ...
    // In iOS 8, [UIScreen bounds] is orientation dependent.  So rather than having different code for iOS 7 and 8, do this hack.
    return NeonMax(rect.size.width, rect.size.height);
}
...
u32 GetScreenAbsoluteHeight()
{
    ...
    // In iOS 8, [UIScreen bounds] is orientation dependent.  So rather than having different code for iOS 7 and 8, do this hack.
    return NeonMin(rect.size.width, rect.size.height);
}

float GetScreenAbsoluteAspect()
{
    return (float)GetScreenAbsoluteWidth() / (float)GetScreenAbsoluteHeight();
}
...
static const NSString* sSuffixTable[] = { @"thousand",
                                          @"million",
                                          @"billion",
                                          @"trillion",
                                          @"quadrillion",
...
{
    int numDigits = 1;
    u64 convertedNumber = inNumber * 10;
    u64 temp = convertedNumber;
    ...
    // Don't do anything until 7 digits (millions).  Below comparison is against 8 since we have one more digit for the tenth spot (we do one decimal place above)
    if (numDigits < 8)
    {
        if (inUseDecimalForShort)
        {
            return [NSString stringWithFormat:@"%llu.%0llu", convertedNumber / 10, convertedNumber % 10];
        }
        else
        {
            return [NSString stringWithFormat:@"%llu", convertedNumber / 10];
        }
    }

    // Truncate in groups of 3s (millions, billions, trillions, etc)
    // Subtract 2 from group since we have to account for the tenths digit.  Normally subtract 1 since if num digits is a multiple of 3 (eg: 999 million), we want group to be the next group down (999 million instead of 0.999 billion)
    int group = (numDigits - 2) / 3;
    int numMajorDigits = numDigits - (3 * group);
    int numMinorDigits = inNumMinorDigits;
    
    int totalDigits = numMajorDigits + numMinorDigits;

    int digitsToTruncate = numDigits - totalDigits;
    u64 roundingNumber = inNumber / pow(10, (digitsToTruncate - 1));
    u64 truncatedNumber = inNumber / pow(10, digitsToTruncate);
    
    roundingNumber -= truncatedNumber * 10;
    ...
    NSCAssert(group < (sizeof(sSuffixTable) / sizeof(NSString*)), @"Exceeded size of the suffix table.");
    
    u64 majorDigits = truncatedNumber / pow(10, numMinorDigits);
    u64 minorDigits = truncatedNumber - majorDigits * pow(10, numMinorDigits);
    
    NSString* formatString = [NSString stringWithFormat:@"%%llu.%%0%dllu %%@", inNumMinorDigits];
    NSString* retString = [NSString stringWithFormat:formatString, majorDigits, minorDigits, sSuffixTable[group - 1]];
...
NSString* NeonFormatLongToLength(u64 inNumber)
{
    ...
    // Don't do anything until 7 digits (millions)
    if (numDigits < 7)
    {
        return [NSString stringWithFormat:@"%llu", inNumber];
    }

    // Truncate in groups of 3s (millions, billions, trillions, etc)
    
    int group = (numDigits - 1) / 3;
    int numMajorDigits = numDigits - (3 * group);
    int numMinorDigits = 3; // 3 decimal places by default
    ...
    NSString* retString = [NSString stringWithFormat:@"%llu.%03llu %@", majorDigits, minorDigits, sSuffixTable[group - 1]];
...
{
    return ((AppDelegate*)[[UIApplication sharedApplication] delegate])->mGlobalMessageChannel;
}
