...
static void CalculateTrimValues(MusicPlayerSongState* inSongState, UInt32 inNumPacketsRead, UInt64* outStartTrim, UInt64* outEndTrim);
...
void MusicPlayerAudioQueueOutputCallback(void* inUserData, AudioQueueRef inAQ, AudioQueueBufferRef inBuffer)
{
    ...
    {        
        ...
        result = AudioQueueEnqueueBufferWithParameters(songState->mAudioQueue, inBuffer,
                                                        ...
                                                        (UInt32)startTrim, (UInt32)endTrim, 0, NULL, NULL, NULL);
...
void CalculateTrimValues(MusicPlayerSongState* inSongState, UInt32 inNumPacketsRead, UInt64* outStartTrim, UInt64* outEndTrim)
{
    UInt64 startTrim = 0;
    UInt64 endTrim = 0;
    ...
    if (inSongState->mPacketTableValid)
    {
        ...
        UInt64 curFrame = inSongState->mCurrentPacket * framesPerPacket;
        UInt64 endFrame = curFrame + (inNumPacketsRead * framesPerPacket);
        ...
        UInt64 lastValidFrame = inSongState->mPacketTableInfo.mNumberValidFrames + startPadding;
